...
void Foam::correctClosedVolumePhiPimple
(
...
)
{
    ...
    // {
    //     if(U.boundaryField()[patchI].type() == regionCoupledMassTransferVelocityValue::typeName)
    //     {
    //         refCast<const regionCoupledMassTransferVelocityValue>
    //         (
    //             U.boundaryField()[patchI]
    //         ).correctClosedVolumePhiPimple(phi,U,p,rAU);
    //     }
    // }
    if (p.needReference())
    {
        ...
        if (mag(adjustableMassOut) > VSMALL)
        {
