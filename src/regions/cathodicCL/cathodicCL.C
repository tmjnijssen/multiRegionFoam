...
void Foam::regionTypes::cathodicCL::updateElectrochemistry()
{
    ...
    j0_ = jStern_*pow(Foam::mag((xO2_()*p_/pRef_)), 0.54)*exp(ER_/RGas_*(1/TRef_ - 1/T_()));

    // galvani potential difference
    deltaPhi_ = phiE_() - phiP_();

    // reversible potential difference
    deltaPhi0_ = -((deltaH_ - T_()*deltaS_)/(2*FConst_)) + (RGas_*T_()/(4*FConst_))*log(Foam::mag(xO2_()*p_/pRef_));
...
void Foam::regionTypes::cathodicCL::updateCurrentDensity()
{
...
void Foam::regionTypes::cathodicCL::meshMotionCorrector()
{
    // do nothing, add as required
}

Foam::scalar Foam::regionTypes::cathodicCL::getMinDeltaT()
{
     return GREAT;
}
