...
void Foam::regionTypes::anodicCL::updateGasSpeciesTransportProperties()
{
    ...
    DEffH2_() = epsilonP_/sqr(tau_)*DH2_*pow(Foam::mag((T_()/TRef_)),1.5)*(pRef_/p_);

    // effective diffusion coefficient vapor
    DEffV_() = epsilonP_/sqr(tau_)*DV_*pow(Foam::mag((T_()/TRef_)),1.5)*(pRef_/p_);
}
...
void Foam::regionTypes::anodicCL::updateAbsorptionDesorption()
{
    ...
    lambdaEq_ = 0.043 + (17.81*Foam::mag(xV_()/xVSat_)) - (39.85*pow(Foam::mag((xV_()/xVSat_)),2)) + (36*pow(Foam::mag((xV_()/xVSat_)),3));
...
void Foam::regionTypes::anodicCL::updateElectrochemistry()
{
    ...
    deltaPhi0_ = -(T_()*deltaS_/(2*FConst_)) - (RGas_*T_()/(2*FConst_))*log(Foam::mag(xH2_()*p_/pRef_));
...
void Foam::regionTypes::anodicCL::updateSourceTerms()
{
    ...
    sT_ = sigma_()*(fvc::grad(phiE_())&fvc::grad(phiE_())) + kappa_()*(fvc::grad(phiP_())&fvc::grad(phiP_())) + (kSorp_/d_/VM_)*(lambdaEq_ - lambda_())*HAD_ + j_*eta_ - (j_/2/FConst_)*T_()*deltaS_;

    // mass source water content in ionomer
    sLambda_ = (kSorp_/d_/VM_)*(lambdaEq_ - lambda_());

    // mass source vapor / phase change & sorption
    sV_ = - (kSorp_/d_/VM_)*(lambdaEq_ - lambda_());
}

void Foam::regionTypes::anodicCL::updateCurrentDensity()
{
...
void Foam::regionTypes::anodicCL::meshMotionCorrector()
{
    // do nothing, add as required
}

Foam::scalar Foam::regionTypes::anodicCL::getMinDeltaT()
{
    return GREAT;
}
